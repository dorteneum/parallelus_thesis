...
chunkfile<-"all.alleles.min12_chunks.out" ## chromopainter chunkcounts file
mcmcfile<-"all.alleles.min12_chunks.mcmc.xml" ## finestructure mcmc file
treefile<-"all.alleles.min12_chunks.mcmcTree.xml" ## finestructure tree file
...
pdf(file="all0Dendrogram.pdf",height=6,width=14)
...
pdf(file="all0_SimplePopulationDendrogram.pdf",height=6,width=12)
...
source("FinestructureRcode/FinestructureLibrary.R")
pdf(file="all_0SimplePairwiseCoincidence.pdf",height=12,width=12)
...
pdf(file="all0_SimpleCoancestry.pdf",height=12,width=12)
...
tmpmat[tmpmat>tmatmax]<-tmatmax # 
pdf(file="all0_PopAveragedCoancestry.pdf",height=12,width=12)
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=0.6,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=0.8))
dev.off()
...
pdf(file="all0_SimpleCoancestry2.pdf",height=12,width=12)
...
pdf("all0_SimpleExample_PCA.pdf",height=16,width=12)
par(mfrow=c(4,3))
