...
setwd('~/Documents/LMU-EES/Thesis/Bioinformatics/structure/fineRADstr/')
source("FinestructureRcode/FinestructureLibrary.R") # read in the R functions, which also calls the needed packages
source("FinestructureRcode/FinestructureDendrogram.R")
...
chunkfile<-"no-out.alleles.min12_chunks.out" ## chromopainter chunkcounts file
mcmcfile<-"no-out.alleles.min12_chunks.mcmc.xml" ## finestructure mcmc file
treefile<-"no-out.alleles.min12_chunks.mcmcTree.xml" ## finestructure tree file

## Additional files that you can extract from finestructure
mappopchunkfile<-"no-outmin12.EMlinked.mapstate.csv" # population-by-population chunkcount file for the populations used in the MAP (i.e tree)
system( paste("fs fs -X -Y -e X2",chunkfile,treefile,mappopchunkfile) )
meancoincidencefile<-"no-outmin12.EMlinked.meancoincidence.csv" # pairwise coincidence, .i.e. proportion of MCMC files where individuals are found in the same 
...
pdf(file="no-outmin12Dendrogram.pdf",height=6,width=14)
...
pdf(file="no-outmin12SimplePopulationDendrogram.pdf",height=6,width=12)
...
pdf(file="no-outmin12SimplePairwiseCoincidence.pdf",height=12,width=12)
...
pdf(file="no-outmin12SimpleCoancestry.pdf",height=12,width=12)
...
tmpmat[tmpmat>tmatmax]<-tmatmax # 
pdf(file="no-outmin12PopAveragedCoancestry.pdf",height=12,width=12)
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=0.6,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=0.8))
dev.off()
...
pdf(file="no-outmin12SimpleCoancestry2.pdf",height=12,width=12)
...
pdf("no-outmin12_PCA.pdf",height=16,width=12)
par(mfrow=c(4,3))
